...
jQuery(function($){
    ...
    dataTableOutsource = $("#dataTable-outsource").DataTable({
        ajax: {
            url: g_baseURL + "/company/outsource/datatable",
            type: 'post',
            data: function(data, settings) {
                data.extra = {
                    outsource_offer_info_id : currentJobTitle,
        ...
        columnDefs: [
            {
                ...
                "render": function ( data, type, row, meta ) {
                    ...
                    (row.selection_status == 1 ? '<p>提案単価</p><a href="' + g_baseURL + '/company/outsource/' + row.id + '"><span class="blue">' + (row.proposal_unit_price!=null ? row.proposal_unit_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') : '-') + '円/月' + '</span>' : '') + '</div>';
                }
            },
            {
                "targets": 1,
                "render": function ( data, type, row, meta ) {
                    return '<div class="padding-box fixed-w200"><a href="#/company/G/' + row.outsource_company_id + '">' + row.outsource_company_name + '</a></div>';
                }
            },
            {
                "targets": 2,
                "render": function ( data, type, row, meta ) {
                    return '<div class="padding-box fixed-w200" data-id="' + row.id +'">'
                        + '<a href="' + g_baseURL + '/company/job_show/G/' + row.outsource_offer_info_id + '">' + row.job_title + '</a><br>'
                        + '<a class="link-detail" href="' + g_baseURL + '/company/job_list?cat=' + row.occupation_category_2 + '">' + row.occupation_category_2 + '</a></div>';
                }
            },
            {
                "targets": 3,
    ...
    $('#dataTable-outsource tbody').on( 'click', 'td', function () {
        ...
        if (/*obj.find('div').length &&*/ index != undefined && index.column > 2) {
            ...
            location.href = g_baseURL + '/company/outsource/' + id;
        }
    });
});
