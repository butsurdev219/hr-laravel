        ...
        <div class="col-md-3" style="padding: 0px;">
            <div class="card mb-2">
                <div class="card-body ta-c pY-10 calendar-ttl">
                    ...
                    <a class="link" style="cursor:pointer; font-size: 90%;" href="{{ '/company/outsource/'.$outsourceJobSeekerApplyMgt->id }}">▶選考詳細に戻る</a>
                </div>
            </div>
            <div class="card mb-2">
                <div class="card-body ta-c">
                    <i class="ti-calendar bold"></i>
                    <b>{{ $jobSeeker->initial }}さんからの候補日程</b><br/><br/>
            ...
            <div class="card mb-2">
                <div class="card-body ta-c pX-0">
                    <i class="ti-calendar bold"></i>
                        ...
                        <a href="/company/outsource/{{ $other->job_seeker_apply_mgt_id }}">{{ $other->interview_candidates_name }}さん</a> {{ date('n/d', strtotime($other->interview_candidates_date)) }} {{ g_enum('week_days', date('w', strtotime($other->interview_candidates_date))) }} {{ substr($other->interview_candidates_from, 0, 5) }}〜{{ substr($other->interview_candidates_to, 0, 5) }}<br>
                        @endforeach
                    </p>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function clickConfirmDate(id, check) {
        ...
        if (confirm(msg)) {
            $.ajax({
                url: "{{ route('company.outsource.send_confirm_interview_date') }}",
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}', 
                ...
                success: function (response) {
                    if (check && response.success == true) {
                        window.location.href = "{{ '/company/outsource/'.$outsourceJobSeekerApplyMgt->id }}";
                    }
                    else {
...
<?php } ?>
        ...
        $("#btn_clearSchedule").click(function() {
            ...
            if (confirm('候補者から提示された日程を全てNGにして別の日程を提示しますか?')) {
                $.ajax({
                    url: "{{ route('company.outsource.send_clear_interview_date') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}', 
                        mgt_id: {{ $outsourceJobSeekerApplyMgt->id }},
                    ...
                    dataType: 'JSON',
                    success: function (response) {
                        window.location.href = "{{ '/company/outsource/calendar2?id='.$outsourceJobSeekerApplyMgt->id }}";
                    },
                    error: function () {
                        console.log("ajax error");
                    }
