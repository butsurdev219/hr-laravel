...
@section('title', 'inCulエージェント 日程調整ページ')
        ...
        <div class="col-md-3" style="padding: 0px;">
            <div class="card mb-2">
                <div class="card-body ta-c pY-10 calendar-ttl">
                    ...
                    <a class="link" style="cursor:pointer; font-size: 90%;" href="{{ '/company/recruit/'.$recruitJobSeekerApplyMgt->id }}">▶選考詳細に戻る</a>
                </div>
            </div>
            <div class="card mb-2">
                <div class="card-body ta-c">
                    <i class="ti-calendar bold"></i>
                    <b>{{ $jobSeeker->last_name . ' ' . $jobSeeker->first_name }}さんからの候補日程</b><br/><br/>
            ...
            <div class="card mb-2">
                <div class="card-body ta-c pX-0">
                    <i class="ti-calendar bold"></i>
                        <b>他の候補者の選考日程</b><br/><br/>
                        <p id="interview_others" class="text">
                        @if (count($otherSchedules) == 0)
                        表示する日程がありません。
                        @endif
                        <!-- →　左サイド　他の候補者の日程も表示（ブッキング防止）。候補者名クリックでその候補者の選考詳細へ -->
                        @foreach ($otherSchedules as $other)
                        <a href="/company/recruit/{{ $other->job_seeker_apply_mgt_id }}">{{ $other->interview_candidates_name }}さん</a> {{ date('n/d', strtotime($other->interview_candidates_date)) }} {{ g_enum('week_days', date('w', strtotime($other->interview_candidates_date))) }} {{ substr($other->interview_candidates_from, 0, 5) }}〜{{ substr($other->interview_candidates_to, 0, 5) }}<br>
                        @endforeach
                    </p>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function clickConfirmDate(id, check) {
        ...
        if (confirm(msg)) {
            $.ajax({
                url: "{{ route('company.recruit.send_confirm_interview_date') }}",
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}', 
                ...
                success: function (response) {
                    if (check && response.success == true) {
                        window.location.href = "{{ '/company/recruit/'.$recruitJobSeekerApplyMgt->id }}";
                    }
                    else {
...
<?php for ($i=0; $i<count($otherSchedules); $i++) { ?>
        formated = moment(new Date('{{ $otherSchedules[$i]->interview_candidates_date }}')).format('YYYY/MM/DD');
        interview_candidates_from = moment(new Date(formated + ' ' + '{{ $otherSchedules[$i]->interview_candidates_from }}')).format('YYYY-MM-DDTHH:mm:ss');
        interview_candidates_to  = moment(new Date(formated + ' ' + '{{ $otherSchedules[$i]->interview_candidates_to }}')).format('YYYY-MM-DDTHH:mm:ss');
        calendar.addEvent({
            title: '{{ $otherSchedules[$i]->interview_candidates_name."さん" }}',
            start: interview_candidates_from,
            end: interview_candidates_to,
            id: {{ $otherSchedules[$i]->id }},
            groupId: '{{ $otherSchedules[$i]->job_seeker_apply_mgt_id }}',
...
<?php } ?>
        ...
        $("#btn_clearSchedule").click(function() {
            ...
            if (confirm('候補者から提示された日程を全てNGにして別の日程を提示しますか?')) {
                $.ajax({
                    url: "{{ route('company.recruit.send_clear_interview_date') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}', 
                        mgt_id: {{ $recruitJobSeekerApplyMgt->id }},
                    ...
                    dataType: 'JSON',
                    success: function (response) {
                        window.location.href = "{{ '/company/recruit/calendar2?id='.$recruitJobSeekerApplyMgt->id }}";
                    },
                    error: function () {
                        console.log("ajax error");
                    }
