...
@section('title', 'inCulエージェント 人材紹介した転職者の選考状況詳細')
        ...
        <div class="col-md-6">
            <div class="card mb-2">
                <div class="card-body ta-c">
                    <a href="{{ '/agent/jobseeker?keywword=' . $jobSeeker->last_name . $jobSeeker->first_name }}" class="seeker_name black">{{ $jobSeeker->last_name . ' ' . $jobSeeker->first_name }}</a> ／ <a href="{{ route('recruit.job_show', ['type' => 'J', 'id' => $offerInfo->id]) }}" class="black">{{ $offerInfo->job_title }}</a>
                </div>
            </div>
            ...
            <div class="card mb-2 card__tabs">
                <div class="card-body">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        ...
                        <div class="shortcut"><a href="/agent/job_show/J/{{ $offerInfo->id }}" class="c-grey-icon">求人情報 ></a></div>
                      </ul>
                      <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active seeker-tab" id="seeker" role="tabpanel" aria-labelledby="seeker-tab">
                            <p class="fee-text">
                                紹介料：{{ $offerInfo->success_reward_calculation_method == 1 ? '年収の ' . floatval($offerInfo->theory_annual_income) . ' ％' : '一律固定報酬 ' . $offerInfo->theory_annual_income_definition . ' 万円' }}
                            </p>
                            <div class="profile">
                                <figure class="figure">
...
<script>
    ...
    let enumRecruitRefusalReasons = @json(g_enum('recruit_refusal_reason'));
...
<script src="{{ url('js/recruit/apply_detail.js') }}"></script>
