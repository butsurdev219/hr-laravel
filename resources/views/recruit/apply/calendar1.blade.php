...
<script type="text/javascript">
    ...
    function clickConfirmDate(id, check) {
        ...
        if (confirm(msg)) {
            $.ajax({
                url: "{{ route('recruit.apply.send_confirm_interview_date') }}",
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}', 
...
<?php } ?>
        ...
        $("#btn_clearSchedule").click(function() {
            ...
            if (confirm('求人企業様から提示された日程を全てNGにして別の日程を提示しますか?')) {
                $.ajax({
                    url: "{{ route('recruit.apply.send_clear_interview_date') }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}', 
                        mgt_id: {{ $recruitJobSeekerApplyMgt->id }},
                    ...
                    dataType: 'JSON',
                    success: function (response) {
                        window.location.href = "{{ '/agent/calendar2?id='.$recruitJobSeekerApplyMgt->id }}";
                    },
                    error: function () {
                        console.log("ajax error");
                    }
